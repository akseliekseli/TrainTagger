...
class TransformerFatJet(JetTagModel):
    ...
    def build_model(self, inputs_shape: tuple, outputs_shape: tuple):
        ...
        for i, (
            ...
            num_dense_layers,
            dim_dense_layers,
        ) in enumerate(self.model_config["transformer_layers"]):
            mha = MultiHeadAttention(
                num_heads, mha_hidden_dim, name="mha_" + str(i + 1)
            )(main, main)
        ...
        pt_regress = Dense(1, name="pT_output", kernel_initializer="lecun_uniform")(
            pt_regress
        )
    ...
    def load(self, out_dir: str = "None"):
        self.jet_model = tf.keras.models.load_model(
            f"{out_dir}/model/saved_model.keras"
        )
