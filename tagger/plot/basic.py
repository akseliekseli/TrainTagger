...
def ROC_taus(y_pred, y_test, class_labels, plot_dir, signal_proc=None):
    ...
    def compute_roc_inputs(y_pred, y_test, signal_indices, background_indices):
        ...
        total_mask = signal_mask | (
            sum(y_test[:, idx] for idx in background_indices) > 0
        )
...
def response(class_labels, y_test, truth_pt_test, reco_pt_test, pt_ratio, plot_dir):
    ...
    # Inclusive response
    uncorrected_response, regressed_response, uncorrected_errors, regressed_errors = (
        get_response(truth_pt_test, reco_pt_test, pt_ratio)
    )
    plot_response(
        uncorrected_response,
        regressed_response,
    ...
    for flavor in class_labels.keys():
        ...
        (
            ...
            truth_pt_test[flavor_selection],
            reco_pt_test[flavor_selection],
            pt_ratio[flavor_selection],
        )
        plot_response(
            uncorrected_response,
            regressed_response,
...
def rms(class_labels, y_test, truth_pt_test, reco_pt_test, pt_ratio, plot_dir):
    ...
    for flavor in class_labels.keys():
        ...
        uncorrected_rms, regressed_rms, uncorrected_rms_err, regressed_rms_err = (
            get_rms(
                truth_pt_test[flavor_selection],
                reco_pt_test[flavor_selection],
                pt_ratio[flavor_selection],
...
def shapPlot(shap_values, feature_names, class_names):
    ...
    ax.set_yticks(
        ...
        [style.INPUT_FEATURE_STYLE[feature_names[i]] for i in feature_order],
        fontsize=30,
    )
    ax.set_xlabel(
        "mean (Shapley value) - (average impact on model output magnitude)", fontsize=30
    )
    plt.tight_layout()
...
def efficiency(y_pred, y_test, reco_pt_test, class_labels, plot_dir):
    ...
    for key in eff_selection.keys():
        ...
        data_eff = ak.to_dataframe(
            ak.Array(
                {
                    "truth": summed_y_true[selection],
                    "pt": reco_pt_test[selection],
                    "pred": summed_y_score[selection],
                }
            )
        )
        ...
        h_wp_loose = histbook.Hist(
            histbook.split("pt", pt_points), histbook.cut("wp_loose")
        )
        h_wp_medium = histbook.Hist(
            histbook.split("pt", pt_points), histbook.cut("wp_medium")
        )
        h_wp_tight = histbook.Hist(
            histbook.split("pt", pt_points), histbook.cut("wp_tight")
        )
        ...
        df_wp_loose["midpoints"] = [
            x[0].mid if isinstance(x[0], pandas.Interval) else np.nan
            for x in df_wp_loose.index
        ]
        df_wp_medium["midpoints"] = [
            x[0].mid if isinstance(x[0], pandas.Interval) else np.nan
            for x in df_wp_medium.index
        ]
        df_wp_tight["midpoints"] = [
            x[0].mid if isinstance(x[0], pandas.Interval) else np.nan
            for x in df_wp_tight.index
        ]
...
def filter_process(test_data, process_dir):
    ...
    train, test, class_labels = load_data(
        os.path.join("signal_process_data", process_dir), percentage=100
    )[:3]
    ...
    unique_test_data, indices_unique_test_data = np.unique(
        test_data, axis=0, return_index=True
    )
    comparison_data = np.concatenate((unique_test_data, process_data), axis=0)
    u, index, counts = np.unique(
        comparison_data, axis=0, return_index=True, return_counts=True
    )
