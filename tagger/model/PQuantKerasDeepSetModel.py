...
from pquant.layers import PQDense, PQActivation, PQConv1D 
...
class PQuantDeepSetModel(JetTagModel):
    ...
    def build_model(self, inputs_shape: tuple, outputs_shape: tuple):
        ...
        for iconv1d, depthconv1d in enumerate(self.model_config['conv1d_layers']):
            main = PQConv1D(filters=depthconv1d, kernel_size=1, name='Conv1D_' + str(iconv1d + 1), activation='relu',**self.common_args)(main)
        ...
        for iclass, depthclass in enumerate(self.model_config['classification_layers']):
            if iclass == 0:
                jet_id = PQDense(depthclass, name='Dense_' + str(iclass + 1) + '_jetID', **self.common_args)(main)
            else:
                jet_id = PQDense(depthclass, name='Dense_' + str(iclass + 1) + '_jetID', activation='relu', **self.common_args)(jet_id)

        jet_id = PQDense(outputs_shape[0], name='Dense_3_jetID',activation='linear',kernel_initializer='lecun_uniform')(jet_id)
        jet_id = PQActivation('softmax', name='jet_id_output')(jet_id)
        ...
        for ireg, depthreg in enumerate(self.model_config['regression_layers']):
            if ireg == 0:
                pt_regress = PQDense(depthreg, name='Dense_' + str(ireg + 1) + '_pT', **self.common_args)(main)
            else:
                pt_regress = PQDense(depthreg, name='Dense_' + str(ireg + 1) + '_pT', activation='relu',**self.common_args)(pt_regress)

        pt_regress = PQDense(1, name='pT_output',
